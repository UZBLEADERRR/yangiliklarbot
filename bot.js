...
if (process.argv.includes('--morning')) {
  const news = await callGemini("Bugungi eng muhim 5 ta dunyo va O'zbekiston yangiliklarini qisqa va tushunarli qilib o'zbek tilida yozib ber.");
...
for (const update of data.result ?? []) {
  ...
  if (callback) {
    ...
    if (callback.data === 'get_news') {
      ...
      const news = await callGemini("Hozirgi vaqtdagi eng shov-shuvli dunyo va O'zbekiston yangiliklarini qisqa, tushunarli va qiziqarli qilib o'zbek tilida yozib ber.");
